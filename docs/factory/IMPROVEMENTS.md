...
| 2026-06-14 | Image | WirePlumber audio priority rules for Framework Desktop (AMD Ryzen AI Max 300): HDMI default sink + internal mic over USB/BT. Shipped as flat wireplumber.conf.d/ rule (hardware-profiles/ is bazzite-specific, requires patched WP + wireplumber-sysconf.service). | common#671 |
| 2026-06-14 | Image | ICC color profile auto-assignment for Framework 13/16: user-session hook (12-framework-color.sh) assigns factory profiles via colormgr on first login. Profiles already existed; hook adds colord binding since DisplayCAL ICC files lack EDID metadata for auto-matching. | common#670 |
| 2026-06-16 | CI | Dep queue unblocked: merged #700 (homebrew/actions), #703 (testsuite), #705 (login-action v4), closed stale #699/#702. Root cause of validate failures on #711/#713/#714/#715: `no-floating-action-tags` hook regex was too strict — matched internal `projectbluefin/` refs that are intentionally floating (lifecycle-caller.yml`@main`, renovate-automerge.yml`@v1`). Fixed by adding negative lookahead exemption for all `projectbluefin/` refs (#716). Branches rebased; automerge queue running. | common#716 |
| 2026-06-20 | Image | umotd built from source via Go builder stage (projectbluefin/motd@v0.2.1); moved to shared layer (all variants, not just bluefin); removed legacy ublue-motd wrapper script. Eliminates floating binary download. | common#744 |
| 2026-06-20 | Lab | Live toggle-testing methodology documented: correct production flow is stable→testing (not testing→stable). WorkflowTemplates `toggle-testing-rebase` + `migration-upgrade-test` created on ghost cluster. BIB SELinux PCRE2 mismatch (10.44 vs 10.46) blocks LTS/dakota builds. Filed testing-lab#220 #221 #222 #223; fixed #221 #222 #223. | testing-lab#224 |
