---
name: bluefin-lts
description: Bluefin LTS variant — use when working in bluefin-lts repo. Contains critical production warnings about disabled ISOs. Always load this skill before touching anything LTS-related.
---
...
Load with: `cat ~/src/skills/bluefin-lts/SKILL.md`
...
- Standard Fedora-based Bluefin (`bluefin`, `aurora`) — use `cat ~/src/skills/bluefin-build/SKILL.md`
- Building or promoting any ISO — LTS ISO is broken; use `cat ~/src/skills/bluefin-iso/SKILL.md` only for HWE
...
> **Note on `bluefin-dx` entries (historical):** `ghcr.io/projectbluefin/bluefin-dx` was a prior SKU that is now **discontinued**. The image may still exist in the registry with old dated tags, but it receives no new builds. Do not roll back or promote it. GDX (`bluefin-gdx`) is the successor for LTS+NVIDIA use cases — it is a distinct product, not "DX on LTS".
...
**gdx caveat:** `bluefin-gdx` publishes less frequently and may not have a dated tag matching the same date as `bluefin`. Check `skopeo list-tags --no-creds docker://ghcr.io/projectbluefin/bluefin-gdx` to find the last good tag before the regression date. **GDX is a distinct product (LTS + NVIDIA driver pre-installed) — it is not "Bluefin DX on LTS".** The old `bluefin-dx` LTS image is discontinued.
